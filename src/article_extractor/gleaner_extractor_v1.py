"""Article extractor for Jamaica Gleaner news source (V1 - CSS-only strategy)."""
...
class GleanerExtractorV1:
    """
    V1 extraction strategy for Jamaica Gleaner articles (CSS-only selectors).

    This is the original implementation before JSON-LD parsing was added.
    Used as fallback when V2 (JSON-LD + CSS) fails.

    Implements ArticleExtractor Protocol for jamaica-gleaner.com.

    HTML Structure (validated 2025-11-28):
    - Article title: <h1 class="title"> → any <h1>
    - Article content: <div class="article-content"> → <div class="field-name-body">
    - Author: <a class="author-term">
    - Published date: <meta property="article:published_time"> → <time datetime> (ISO 8601 format)
    """

    def extract(self, html: str, url: str) -> ExtractedArticleContent:
        """
        Extract article content from Gleaner HTML using CSS-only selectors (V1 strategy).
